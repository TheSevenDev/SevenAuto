...
const FORMAT_VIDEO = ['m4v', 'avi', 'mpg', 'mp4', 'webm', 'mov', 'mkv'];
...
export function fileData(file: IMedia | ExtendFile | string) {
  // Url
  ...
  if ('id' in file) {
    return {
      key: file.id,
      preview: getMediaUrl(file),
      name: file.title,
      type: file.ext,
      size: file.size,
      lastModified: file.updatedAt,
      lastModifiedDate: file.updatedAt,
    };
  }
